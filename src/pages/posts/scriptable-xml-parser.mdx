...
image:
  url: "shareable-widget.jpg"
  alt: "Sharable widget example"
tags: ["javascript", "scriptable", "widget"]
---

import CenterText from "@components/posts/CenterText.astro";

I originally made an XML parser in Scriptable for my [HTML Widget](https://github.com/Normal-Tangerine8609/Scriptable-HTML-Widget) project to replace the previous one. My goal was to make a scriptable native XML parser that was fast, small and error free. But XML parsing can be used for so much more. In this article, I walk you through how I made the parser and what we can use it for.
...
So let's start on a simple XML parser for Scriptable so we can create widgets for RSS data!
...
Notice in this example how most tags are transformed so their tag name is a key in the JSON, and their content is changed to the key's value. If the tag content is plain text, the value will be a string. If the tag has children tags, the value will be a object with those children values stored. If a tag has multiple children of the same tag name, the value will be an array.
...
Here is a visual example of when the fuctions are run:

```txt
┌────────────────────────────────────┐ ┌─────────────────────────┐ ┌──────┐
│ <a href="https://www.google.com/"> │ │ This is the inner text. │ │ <a/> │
└────────────────────────────────────┘ └─────────────────────────┘ └──────┘
...
// ...
parser.foundCharacters = (text) => {
  target.innerText += target.innerText === "" ? text.trim() : " " + text.trim();
};
// ...
```
...
And that finished the first part of the parser. If you test it with the test XML below and log `main`, you should get the following jumble of unworkable JSON.
...
<CenterText>
  <small>
    Image designed by [Anand K
    Parmar](https://medium.com/u/bafde9c53431?source=post_page-----d56328450846--------------------------------)
  </small>
</CenterText>
...
2. Because we don't collect the attributes for the nodes, our output JSON does not cover the full XML, but it is easier to use and collects most of the information you will ever need.
...
![Shareable Widget](/Portfolio/posts/shareable-widget.jpg)
